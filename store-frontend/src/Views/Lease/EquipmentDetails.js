...
const EquipmentDetails = ({ route }) => {
  ...
  const [editableSportCategory, setEditableSportCategory] = useState(
    equipment.sportCategory
  );
  const [editableDeliveryType, setEditableDeliveryType] = useState(
  ...
  const [isDeliveryTypeDropdownVisible, setIsDeliveryTypeDropdownVisible] =
    useState(false);
  ...
  const [selectedAvailable, setSelectedAvailable] = useState(
    initialAvailableString
  );
  const [editableAvailable, setEditableAvailable] = useState(
    equipment.availableStatus
  );
  ...
  useEffect(() => {
    ...
    if (equipment.pickupLocation) {
      setPickupLocation({
        latitude: equipment.pickupLocation.latitude,
        longitude: equipment.pickupLocation.longitude,
      });
    }
  }, [sportCategories, equipment.pickupLocation]);
  ...
  const renderPickupLocation = () => {
    if (editableDeliveryType == "pickup") {
      return (
  ...
  const handleEdit = () => {
    if (isEditMode) {
      ...
      const itemObject = {
        ...
        pickupLocation:
          editableDeliveryType == "delivery" ? null : pickupLocation,
        type: equipment.type,
        id: equipment.id,
      };
  ...
  const editEquipment = async (equipmentId, equipmentItem) => {
    setIsLoading(true);
    try {
      const response = await EquipmentController.editEquipment(
        equipmentId,
        equipmentItem
      );
    ...
    <ScrollView style={styles.container}>
      <View style={styles.androidFooterFix}>
        ...
        <Card style={styles.card}>
          ...
          {isEditMode && <Button title="Change Image" onPress={selectImage} />}
          {isEditMode ? (
            <TextInput
              value={editableTitle}
        ...
        {isEditMode ? (
          ...
          >
            ...
            <View style={styles.timeContainer}>
              ...
              <Image
                source={
                  isSportDropdownVisible ? upward_cevron : downward_cevron
                }
                style={styles.cevron}
              />
            </View>
          ...
          <Card style={styles.card}>
            <Text style={styles.price}>
              Sport Category: {editableSportCategory}
            </Text>
          </Card>
        )}
        ...
        {isEditMode ? (
          ...
          <Card style={styles.card}>
            <Text style={styles.price}>
              Available: {editableAvailable ? "Yes" : "No"}
            </Text>
          </Card>
        )}
        ...
        {isEditMode ? (
          <TouchableOpacity
            style={styles.card}
            onPress={() =>
              setIsDeliveryTypeDropdownVisible(!isDeliveryTypeDropdownVisible)
            }
            activeOpacity={1}
          >
          ...
          <Card style={styles.card}>
            <Text style={styles.price}>
              Delivery Type: {editableDeliveryType}
            </Text>
          </Card>
        )}
